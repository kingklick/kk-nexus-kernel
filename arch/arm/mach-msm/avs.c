...
struct clkctl_acpu_speed acpu_max_vdd_tbl[] = {
	{  19200, VOLTAGE_MIN, 1050 },
	{ 128000, VOLTAGE_MIN, 1050 },
	{ 245000, VOLTAGE_MIN, 1050 },
	{ 256000, VOLTAGE_MIN, 1050 },
	{ 384000, VOLTAGE_MIN, 1050 },
	{ 422400, VOLTAGE_MIN, 1050 },
	{ 460800, VOLTAGE_MIN, 1050 },
	{ 499200, VOLTAGE_MIN, 1075 },
	{ 537600, VOLTAGE_MIN, 1100 },
	{ 576000, VOLTAGE_MIN, 1100 },
	{ 614400, VOLTAGE_MIN, 1125 },
	{ 652800, VOLTAGE_MIN, 1150 },
	{ 691200, VOLTAGE_MIN, 1175 },
	{ 729600, VOLTAGE_MIN, 1200 },
	{ 768000, VOLTAGE_MIN, 1200 },
	{ 806400, VOLTAGE_MIN, 1225 },
	{ 844800, VOLTAGE_MIN, 1250 },
	{ 883200, VOLTAGE_MIN, 1275 },
 ...
 */
static void avs_update_voltage_table(short *vdd_table, int min_voltage, int max_voltage)
{
	u32 avscsr;
	int cpu;
	int vu;
	...
	if ((cpu == 3) || (vu == 3) || (l2 == 3)) {
		...
		for (i = 0; i < avs_state.freq_cnt; i++) {
			...
			if (vdd_table[i] > max_voltage)
				vdd_table[i] = max_voltage;
		}
	} else if ((cpu == 1) && (l2 == 1) && (vu == 1)) {
		//if ((cur_voltage - VOLTAGE_STEP >= VOLTAGE_MIN) &&
		if ((cur_voltage - VOLTAGE_STEP >= min_voltage) &&
...
static short avs_get_target_voltage(int freq_idx, bool update_table)
{
	...
	if (update_table)
		avs_update_voltage_table(vdd_table, acpu_max_vdd_tbl[freq_idx].min_vdd, acpu_max_vdd_tbl[freq_idx].max_vdd);

	//return vdd_table[freq_idx];
	voltage = vdd_table[freq_idx];

	if (voltage > acpu_max_vdd_tbl[freq_idx].max_vdd)
	{
		voltage = acpu_max_vdd_tbl[freq_idx].max_vdd;
	}
	if( voltage < acpu_max_vdd_tbl[freq_idx].min_vdd)
	{
		voltage = acpu_max_vdd_tbl[freq_idx].min_vdd;
	}
